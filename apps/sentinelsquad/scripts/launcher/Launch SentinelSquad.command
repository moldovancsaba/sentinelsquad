...
SENTINELSQUAD_WORKER_AGENT_KEY="${SENTINELSQUAD_WORKER_AGENT_KEY:-Gwen}"
SENTINELSQUAD_WORKER_RUNTIME="${SENTINELSQUAD_WORKER_RUNTIME:-LOCAL}"
SENTINELSQUAD_WORKER_MODEL="${SENTINELSQUAD_WORKER_MODEL:-deepseek-r1:1.5b}"
...
if [[ "$SENTINELSQUAD_WORKER_AUTOSTART" == "1" ]]; then
  ...
  "$NPM_BIN" run worker >"$WORKER_LOG" 2>&1 &
  worker_pid=$!
  $SLEEP_BIN 2
...
"$OPEN_BIN" "$SENTINELSQUAD_APP_URL" >/dev/null 2>&1 || true
