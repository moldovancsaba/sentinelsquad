...
function resolveTaskMemoryConfig(env = process.env) {
  const queryMaxChars = clampInt(env.SENTINELSQUAD_MEMORY_QUERY_MAX_CHARS, 280, 80, 600);
  const defaultDocumentLimit = clampInt(env.SENTINELSQUAD_MEMORY_DOCUMENT_LIMIT, 80, 10, 200);
  const defaultMaxSnippets = clampInt(env.SENTINELSQUAD_MEMORY_DEFAULT_MAX_SNIPPETS, 4, 1, 8);
  const defaultSnippetMaxChars = clampInt(
    env.SENTINELSQUAD_MEMORY_DEFAULT_SNIPPET_MAX_CHARS,
    320,
    80,
    800
  );
  const indexDocumentMaxChars = clampInt(
    env.SENTINELSQUAD_MEMORY_INDEX_DOC_MAX_CHARS,
    1200,
    120,
    4000
  );
...
function evaluateTaskMemoryPolicy(request, config = resolveTaskMemoryConfig()) {
  ...
  if (request.scope !== "THREAD") {
    return {
      ...
      reason: `Memory scope "${request.scope}" is denied. Only THREAD scope is allowed.`
    };
  }
  if (!request.threadId) {
    return {
