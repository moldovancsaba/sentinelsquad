...
function main() {
  ...
  const summary = {
    runId: `sentinelsquad-security-policy-baseline-e2e-${new Date().toISOString()}`,
    stages: {}
  };
...
try {
  ...
  console.error("[sentinelsquad-security-policy-baseline-e2e] failed:", error.message || error);
  process.exitCode = 1;
}
