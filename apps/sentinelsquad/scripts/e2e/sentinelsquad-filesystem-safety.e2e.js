  ...
  await executeFilesystemToolCall(
    {
      ...
      args: {
        ...
        content: "sentinelsquad filesystem safety harness\nstatus: ok\n"
      }
    },
    context
  );
...
async function main() {
  ...
  const summary = {
    runId: `sentinelsquad-filesystem-safety-e2e-${new Date().toISOString()}`,
    stages: {}
  };

  const workspaceRaw = await fsp.mkdtemp(path.join(os.tmpdir(), "sentinelsquad-fs-safety-workspace-"));
  const outsideRaw = await fsp.mkdtemp(path.join(os.tmpdir(), "sentinelsquad-fs-safety-outside-"));
...
main().catch((error) => {
  console.error("[sentinelsquad-filesystem-safety-e2e] failed:", error.message || error);
  process.exitCode = 1;
});
