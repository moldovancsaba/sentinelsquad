...
async function stageToolCallProtocol() {
  const envelope = {
    protocol: "sentinelsquad.tool-call",
    version: "1.0",
    mode: "SEQUENTIAL",
    calls: [
      {
...
async function stageFilesystemAndShell(workspaceRoot) {
  const fsContext = await resolveFilesystemToolContext({
    cwd: workspaceRoot,
    env: { SENTINELSQUAD_WORKSPACE_ROOT: workspaceRoot }
  });

  await executeFilesystemToolCall(
    {
      tool: "filesystem.write",
      args: { path: "rehearsal/e2e.txt", content: "sentinelsquad-e2e" }
    },
    fsContext
  );
  ...
  assert(readResult.answer.includes("sentinelsquad-e2e"), "filesystem read/write stage failed");
...
async function stageGitFlow(tempRoot) {
  ...
  run("git", ["config", "user.email", "sentinelsquad-e2e@example.com"], repo);

  await fsp.writeFile(path.join(repo, "README.md"), "# sentinelsquad e2e\n", "utf8");
...
async function main() {
  ...
  const summary = {
    runId: `sentinelsquad-e2e-${new Date().toISOString()}`,
    stages: {}
  };

  const workspaceRaw = await fsp.mkdtemp(path.join(os.tmpdir(), "sentinelsquad-e2e-workspace-"));
  const gitRaw = await fsp.mkdtemp(path.join(os.tmpdir(), "sentinelsquad-e2e-git-"));
...
main().catch((error) => {
  console.error("[sentinelsquad-e2e] failed:", error.message || error);
  process.exitCode = 1;
});
