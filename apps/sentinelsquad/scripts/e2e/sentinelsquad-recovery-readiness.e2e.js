...
async function main() {
  ...
  const runId = `sentinelsquad-recovery-readiness-e2e-${new Date().toISOString()}`;
  ...
  const root = await fsp.mkdtemp(path.join(os.tmpdir(), "sentinelsquad-recovery-drill-"));
...
main().catch((error) => {
  console.error("[sentinelsquad-recovery-readiness-e2e] failed:", error.message || error);
  process.exitCode = 1;
});
