...
export async function bootstrapSentinelSquadProjectAction() {
  ...
  const repoOwner = process.env.SENTINELSQUAD_TASK_REPO_OWNER || "moldovancsaba";
  const repoName = process.env.SENTINELSQUAD_TASK_REPO_NAME || "sentinelsquad";
  ...
  if (process.env.SENTINELSQUAD_ENABLE_PRODUCT_OPTION_SYNC === "1") {
  ...
  await upsertProjectSetting({
    ...
    vars: [
      { key: "APP_PATH", value: path.join(repoRoot, "apps", "sentinelsquad") },
      { key: "ROADMAP_DOC", value: path.join(repoRoot, "docs", "SENTINELSQUAD_STATUS_BRAINDUMP.md") },
      { key: "HANDOVER_DOC", value: path.join(repoRoot, "docs", "SENTINELSQUAD_HANDOVER.md") },
      { key: "SETUP_DOC", value: path.join(repoRoot, "docs", "SENTINELSQUAD_SETUP.md") }
    ]
  }, {
    auditContext: {
      actorRole: `RBAC_${auth.role}`,
