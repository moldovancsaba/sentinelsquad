...
function readSecret(explicit?: string): string {
  const secret =
    (explicit || process.env.SENTINELSQUAD_TOOL_APPROVAL_SECRET || process.env.NEXTAUTH_SECRET || "")
      .trim();
  if (!secret) {
    throw new Error(
      "Tool approval secret is not configured. Set SENTINELSQUAD_TOOL_APPROVAL_SECRET (or NEXTAUTH_SECRET)."
    );
  }
  return secret;
}
