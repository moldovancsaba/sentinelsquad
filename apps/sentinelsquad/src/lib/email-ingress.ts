...
function resolveSenderAuth(senderEmail: string): SenderAuthResult {
  const blocked = parseAddressList(process.env.SENTINELSQUAD_EMAIL_BLOCKED_SENDERS);
  const trusted = parseAddressList(process.env.SENTINELSQUAD_EMAIL_TRUSTED_SENDERS);
  const requireTrusted = isTruthy(process.env.SENTINELSQUAD_EMAIL_REQUIRE_TRUSTED, true);
  ...
  if (requireTrusted && !trusted.has(email)) {
    return {
      allowed: false,
      reason: "Sender is not in SENTINELSQUAD_EMAIL_TRUSTED_SENDERS."
    };
  }
...
function computeRetryDelayMs(attempt: number) {
  const base = clampInt(process.env.SENTINELSQUAD_EMAIL_RETRY_BASE_MS, 1_000, 100, 60_000);
  const max = clampInt(process.env.SENTINELSQUAD_EMAIL_RETRY_MAX_MS, 15_000, base, 300_000);
