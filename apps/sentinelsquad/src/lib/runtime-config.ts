...
export async function resolveRuntimeConfigForTask(params: {
  ...
  const effective: RuntimeConfigEffective = {
    ...
    requestTimeoutMs: clampTimeout(process.env.SENTINELSQUAD_WORKER_REQUEST_TIMEOUT_MS, 60_000)
  };
  ...
  sourceChain.push({
    ...
    appliedKeys:
      runtime === "LOCAL"
        ? ["OLLAMA_BASE_URL", "OLLAMA_MODEL", "SENTINELSQUAD_WORKER_REQUEST_TIMEOUT_MS"]
        : [
            "OPENAI_BASE_URL",
            "OPENAI_MODEL",
            "OPENAI_API_KEY",
