...
const DEFAULT_COMMAND_PROFILES_PATH = path.resolve(process.cwd(), ".sentinelsquad", "ide-command-profiles.json");
const DEFAULT_UNSAFE_CONFIRM_PHRASE = "I UNDERSTAND THIS ENABLES FULL LOCAL ACCESS";

function workspaceRoot() {
  const root = (process.env.SENTINELSQUAD_IDE_ROOT || DEFAULT_IDE_ROOT).trim();
...
export function getIdeUnsafeModeInfo() {
  const enabled = String(process.env.SENTINELSQUAD_IDE_UNSAFE_FULL_ACCESS || "").trim() === "1";
  const requiredPhrase = String(
    process.env.SENTINELSQUAD_IDE_UNSAFE_CONFIRM_PHRASE || DEFAULT_UNSAFE_CONFIRM_PHRASE
  ).trim();
...
async function readCommandProfileConfig(): Promise<CommandProfileConfig> {
  const file = process.env.SENTINELSQUAD_IDE_COMMAND_PROFILES || DEFAULT_COMMAND_PROFILES_PATH;
  try {
