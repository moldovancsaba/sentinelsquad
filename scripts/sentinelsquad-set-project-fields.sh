#!/usr/bin/env bash
# Set SentinelSquad Board project fields (Status, Agent, Product, Type, Priority) for an issue.
...
# Usage: ./scripts/sentinelsquad-set-project-fields.sh ISSUE_NUMBER [--status STATUS] [--agent AGENT] [--product PRODUCT] [--type TYPE] [--priority PRIORITY]
# Env overrides (only for fields you pass): MVP_STATUS, MVP_AGENT, MVP_PRODUCT, MVP_TYPE, MVP_PRIORITY

set -e
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DEFAULTS_FILE="$SCRIPT_DIR/sentinelsquad-defaults.env"
PROMPT_VALIDATOR="$SCRIPT_DIR/sentinelsquad-validate-prompt-package.js"
PROJECT_OWNER="${MVP_PROJECT_OWNER:-moldovancsaba}"
REPO_NAME="${MVP_REPO:-sentinelsquad}"
...
if [ -f "$DEFAULTS_FILE" ]; then
  set -a
  # shellcheck source=./sentinelsquad-defaults.env
...
if [ "${MVP_SKIP_HANDOVER_GATE:-0}" != "1" ] && [ "$(echo "$STATUS" | tr '[:upper:]' '[:lower:]')" = "done" ] && [ "$(echo "$CURRENT_STATUS" | tr '[:upper:]' '[:lower:]')" != "done" ]; then
  HANDOVER_VALIDATOR="$SCRIPT_DIR/sentinelsquad-validate-handover.sh"
